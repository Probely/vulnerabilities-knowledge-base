class CircularProgressBar {
  constructor(options) {
    ...
    const { pie, percent, color, strokeWidth, opacity, number, size, fontSize, fontWeight, fontColor, shadowColor } = this.options;
    this.pie = pie;
    this.pieElement = document.querySelector(`.${pie}`);
    this.percent = percent || 65;
    this.color = color || '#00a1ff';
    this.shadowColor = shadowColor || "#232323";
  ...
  circularProgressBar() {
    ...
    this.pieElement.setAttribute('style', `position: relative; border-radius: 50%; width: ${this.size}px; height: ${this.size}px; box-shadow: inset 0px 0px ${this.strokeWidth}px ${this.strokeWidth}px rgba(${this.hexTorgb(this.shadowColor)}, ${this.opacity})`);
  ...
  percentElementUpdate(numbers) {
    ...
    percentNumber.innerHTML = `${numbers.toString()[0]}.${numbers.toString()[1] || 0}`;
  }
